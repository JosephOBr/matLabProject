...
% READ DATA FROM FILE
data = getSeries(["Time","R.ASISX","R.ASISY","R.Heel.BottomX","R.Heel.BottomY","L.Heel.BottomX","L.Heel.BottomY"]);
% Save data to variables
[timeData, rPelvisX,rPelvisY,rHeelX, rHeelY,lHeelX,lHeelY] = data{:};


fig = figure('Color','w');
ax  = axes('Parent', fig);
hold(ax, 'on');          % IMPORTANT: prevents plot() from clearing prior graphics
axis(ax, [1500 4000 0 2500]);
% axis(ax, 'equal');
% grid(ax, 'on');
...
pauseTime = 0.005;
for frame = 1:min(4500, numel(rHeelX))
